...
import serviceAccountKey from "./reactjs-blog-app-21291-firebase-adminsdk-em0hw-79e1f4d0b6.json" assert { type: "json" };
...
server.post("/get-upload-url", async (req, res) => {
  try {
    ...
    return res.status(200).json({ uploadUrl: cloudinaryRes.url });
  } catch (error) {
    console.log(error);
...
server.post("/google-auth", (req, res) => {
  ...
  getAuth()
    ...
    .then(async (decodedUser) => {
      ...
      picture = picture.replace("s98-c", "s384-c");
    ...
    .catch((err) => {
      return res.status(500).json({
        error: "Failed to authenticate with google.Try with any other account",
      });
    });
});
  ...
  User.findOne({ _id: req.user })
    .then((user) => {
      if (user.google_auth) {
        return res.status(403).json({
          error:
            "You can't change the password as you have been logged in through google",
        });
      }
      ...
      bcrypt.compare(
        ...
        (err, result) => {
          if (err) {
            return res.status(500).json({
              error:
                "Some error occured while changing the password,please try again letter",
            });
          }
          ...
          bcrypt.hash(newPassword, 10, (err, hashed_password) => {
            User.findOneAndUpdate(
              ...
              .catch((err) => {
                return res.status(500).json({
                  error:
                    "Some error occured while saving the new password,please try again letter",
                });
              });
          });
        }
    ...
    for (let i = 0; i < socialLinksArr.length; i++) {
      if (social_links[socialLinksArr[i]].length) {
        ...
        if (
          ...
          return res.status(403).json({
            error: `${socialLinksArr[i]} link is invalid.You must enter a full domain name`,
          });
        }
      }
    }
    ...
    if (replying_to) {
      ...
      if (notification_id) {
        Notification.findOneAndUpdate(
          ...
          console.log("notifiacation updated");
        });
      }
    }
